...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        printf("Usage: ImageY.pgm ImageCr.pgm ImageCb.pgm \n");
        exit(1);
    }
    ...
    for (int i = 0; i < nH; i++) {
        for (int j = 0; j < nW; j++) {
            R[3 * (i * nW + j)] = Y[i * nW + j] + 1.402 * (Cr[i /2 * (nW / 2) + j/2] - 128);
    ...
    ecrire_image_ppm("NouveauR", R, nH, nW);
    ecrire_image_ppm("NouveauG", G, nH, nW);
    ecrire_image_ppm("NouveauB", B, nH, nW);
